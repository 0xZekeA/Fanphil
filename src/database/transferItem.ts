...
export async function addTransferItem(
  ...
  try {
    ...
    const { data: sellerInventoryItem } = await supastash
...
export async function updateTransferItem(
  ...
  try {
    const { data: existingItem } = await supastash
      .from("transfer_items")
      .select<TransferItem>("*")
      .eq("id", id)
      .single()
...
export const deleteTransferItem = async (id: string) => {
  try {
    ...
    const { data: itemInInventory } = await supastash
      .from("inventory")
      .select<Inventory>("*")
      .eq("id", inventoryId)
      .is("deleted_at", null)
    ...
    const { data: itemInSellersInventory } = await supastash
      .from("sellers_inventory")
      .select<SellersInventory>("*")
